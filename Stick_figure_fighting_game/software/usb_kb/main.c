...
#include "lcp_cmd.h"
#include "lcp_data.h"
	volatile unsigned int *KKK = (unsigned int*)0x10;
...
int main(void)
{
	...
	while(1)
	{
		...
		IOWR(KEYCODE_BASE, 0, keycode & 0xffff);
			...
			IO_write(HPI_ADDR,0x0520); //the start address
			keycode1 = IO_read(HPI_DATA);
			printf("\n second two keycode values are %04x\n",keycode1);
			*KKK=keycode1;
		/*	printf("\n second two keycode values are %04x\n",keycode1);
			//IOWR(KEYCODE_BASE, 0, (keycode<<16&0xffff0000)|keycode1);
			keycode=(0x11110000)|keycode;
			printf("\n second two keycode values are %04x\n",keycode1);
			*KKK=keycode;*/
		/*	IO_write(HPI_ADDR,0x0522); //the start address
				keycode3 = IO_read(HPI_DATA);
